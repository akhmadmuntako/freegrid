...
public class CheckableView extends FrameLayout implements View.OnClickListener {
    ...
    /**
     * Container of the checked/normal view
     */
    private View mViewContainer;
    /**
     * ImageView which holds the checked view
     */
    private View mCheckedImageView;
    /**
     ...
     */
    public View getCheckedImageView() {
...
//        getCheckedImageView().setColorFilter(mCheckedImageColor, PorterDuff.Mode.SRC_IN);
     ...
     */
    public View getNormalImageView() {
...
//        getNormalImageView().setColorFilter(mNormalImageColor, PorterDuff.Mode.SRC_IN);
    ...
    public void setLabel(String label) {
        ...
        if (getLabel() != null) {
            ...
            ((FrameLayout.LayoutParams) mViewContainer.getLayoutParams()).bottomMargin = getResources().getDimensionPixelSize(R.dimen.checkable_image_container_margin) * 2;
        } else {
            getLabelView().setVisibility(View.GONE);
            ((FrameLayout.LayoutParams) mViewContainer.getLayoutParams()).bottomMargin = getResources().getDimensionPixelSize(R.dimen.checkable_image_container_margin);
        }
     ...
     * @see #setCheckmarkPosition(com.example.gembong.myapplication.CheckableView.CheckPosition)
     */
     ...
     * @see com.example.gembong.myapplication.CheckableView.CheckPosition
     */
    ...
    private void init() {
        ...
        mViewContainer = findViewById(R.id.checkable_view_image_container);
        mCheckedImageView = (View) findViewById(R.id.checkable_view_checked_image);
        mNormalImageView = (View) findViewById(R.id.checkable_view_normal_image);
...
//        mLabelView = (TextView) findViewById(R.id.checkable_view_label);
    ...
    private void initImageContainerBackground() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            mViewContainer.setBackground(transitionDrawable);
        } else {
            mViewContainer.setBackgroundDrawable(transitionDrawable);
        }
    }
    ...
    private void animateChecked(boolean animate) {
        if (animate) {
            ...
            set.playTogether(
                    getImageTranslateAnimator(getCheckedImageView(), -mViewContainer.getMeasuredHeight(), 0),
                    getImageTranslateAnimator(getNormalImageView(), 0, mViewContainer.getMeasuredHeight()));
            set.start();
            ...
            ((TransitionDrawable) mViewContainer.getBackground()).startTransition(0);
        }
    }
    ...
    private void animateUnchecked(boolean animate) {
        if (animate) {
            AnimatorSet set = new AnimatorSet();
            set.playTogether(
                    getImageTranslateAnimator(getCheckedImageView(), 0, -mViewContainer.getMeasuredHeight()),
                    getImageTranslateAnimator(getNormalImageView(), mViewContainer.getBottom(), 0));
            set.start();
            ...
            ((TransitionDrawable) mViewContainer.getBackground()).resetTransition();
        }
    }
    ...
    private void startBackgroundTransition(boolean isChecking) {
        if (isChecking) {
            ((TransitionDrawable) mViewContainer.getBackground()).startTransition(mIsInflated ? getAnimationDuration() / 2 : 0);
        } else {
            ((TransitionDrawable) mViewContainer.getBackground()).reverseTransition(mIsInflated ? getAnimationDuration() / 2 : 0);
        }
    }
